...
_DEFAULT_FONTS = {_DEFAULT_FONT_SIZE: ImageFont.truetype(_DEFAULT_FONT_PATH, _DEFAULT_FONT_SIZE) if os.path.isfile(
...
def _get_marker_range_text(markers, marker_cmap):
    if markers:  # not None and not empty
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            min_marker_score_text = np.nanmin([marker['score'] for marker in markers]) if use_marker_range else None
            max_marker_score_text = np.nanmax([marker['score'] for marker in markers]) if use_marker_range else None
        ...
        marker_range_text = ("Markers: Constant: %0.3f" % min_marker_score_text if is_marker_score_constant else
                             "Markers: Min (%s): %0.3f Max (%s): %0.3f" % (
                                 min_marker_color, min_marker_score_text, max_marker_color,
                                 max_marker_score_text)) if use_marker_range else None
    else:
...
def _get_draw_text(text, text_off, image_range_text, image_range_text_off, marker_range_text, marker_range_text_off):
    draw_text = ""
    if text is not None and not text_off:
        draw_text += text

        if (marker_range_text is not None and not marker_range_text_off) or not image_range_text_off:
            draw_text += "\n"
...
def np2d(arr, colorize=True,
    ...
    draw_text = _get_draw_text(text, text_off, image_range_text, image_range_text_off, marker_range_text,
    ...
    if markers:  # not None and not empty
        ...
        add_markers_to_img(img=img, markers=markers, marker_radius=marker_radius, marker_text_off=marker_text_off,
                           cmap=marker_cmap, ignore_scores=ignore_marker_scores, min_score=min_marker_score,
...
def add_text_to_img(img, text,
    ...
    if bbox_color is not None or bbox_stroke is not None:
        ...
        bbox = ([(xy_lefttop[0] - bbox_space, xy_lefttop[1]),
                 (text_size[0] + xy_lefttop[0] + bbox_space + 1, text_size[1] + xy_lefttop[1] + bbox_space + 1)])
...
def add_markers_to_img(img, markers, marker_radius=None, marker_text_off=False, cmap=_DEFAULT_CMAP, ignore_scores=False,
    ...
    for marker in markers:
        ...
        if use_scores:
            if np.isfinite(marker['score']):
                marker_color = get_cmap_color(value=marker['score'], lower_thresh=min_score, upper_thresh=max_score,
                                              cmap=cmap)
        ...
        draw.ellipse((max(0, xy_pos[0] - r), max(0, xy_pos[1] - r), xy_pos[0] + r, xy_pos[1] + r), fill=marker_color)
        ...
        if not marker_text_off and (show_desc_text or show_score_text):
            ...
            add_text_to_img(img=img, text=text, xy_leftbottom=(xy_pos[0], img.height - xy_pos[1]), x_rel_shift=-0.5,
...
def _visualize_np_flow_array(arr, text=None,
    ...
    # Filter out all values that are somehow invalid and set them to 0:
    arr, invalid_mask, invalid_values_mask, _, _, _, _, _ = invalidate_np_array(arr)  # add clipping here if you want
    invalid_flow_values_mask = np.logical_and(invalid_values_mask[:, :, 0], invalid_values_mask[:, :, 1])
