...
def log_image_N1or3HW(writer, desc, tensor, step, idx=None, full_batch=False, colorize=True,
    ...
    if tensor.ndim == 2:
        tensor = np.expand_dims(tensor, 0) if isinstance(tensor, np.ndarray) else torch.unsqueeze(tensor, 0)
    if tensor.ndim == 3:
        tensor = np.expand_dims(tensor, 0) if isinstance(tensor, np.ndarray) else torch.unsqueeze(tensor, 0)

    ch = tensor.shape[1]
    if ch == 1:
        log_image_N1HW(writer=writer, desc=desc, tensor=tensor, step=step,
                       idx=idx, full_batch=full_batch, colorize=colorize,
        ...
        log_image_NCHW(writer=writer, desc=desc, tensor=tensor, step=step,
                       idx=idx, full_batch=full_batch, channels='FLOW',
                       clipping=clipping, upper_clipping_thresh=upper_clipping_thresh,
                       lower_clipping_thresh=lower_clipping_thresh,
                       mark_clipping=mark_clipping, clipping_color=clipping_color,
                       invalid_values=invalid_values, mark_invalid=mark_invalid, invalid_color=invalid_color,
                       text=text, gray=(not colorize),
                       markers=markers, marker_radius=marker_radius, marker_text_off=marker_text_off,
                       marker_cmap=marker_cmap,
                       ignore_marker_scores=ignore_marker_scores, min_marker_score=min_marker_score,
                       max_marker_score=max_marker_score,
        ...
        raise ValueError("image_N1or3HW can only be used with a channel dimension of 1 or 3, not {}.".format(ch))


def log_image_N1HW(writer, desc, tensor, step, idx=None, full_batch=False, colorize=True,
                   clipping=False, upper_clipping_thresh=None, lower_clipping_thresh=None,
                   mark_clipping=False, clipping_color=None,
                   invalid_values=None, mark_invalid=False, invalid_color=None,
                   text=None, cmap=None,
    ...
    if tensor.ndim == 2:
        tensor = np.expand_dims(tensor, 0) if isinstance(tensor, np.ndarray) else torch.unsqueeze(tensor, 0)
    if tensor.ndim == 3:
        tensor = np.expand_dims(tensor, 1) if isinstance(tensor, np.ndarray) else torch.unsqueeze(tensor, 1)
...
def log_image_NCHW(writer, desc, tensor, step, idx=None, full_batch=False,
                   channels='RGB', text=None, gray=False,
                   clipping=False, upper_clipping_thresh=None, lower_clipping_thresh=None,
                   mark_clipping=False, clipping_color=None,
                   invalid_values=None, mark_invalid=False, invalid_color=None,
                   markers=None, marker_radius=None, marker_text_off=False, marker_cmap=None,
    ...
    img = np3d(arr=tensor, channels=channels, text=text, gray=gray,
               clipping=clipping, upper_clipping_thresh=upper_clipping_thresh,
               lower_clipping_thresh=lower_clipping_thresh,
               mark_clipping=mark_clipping, clipping_color=clipping_color,
               invalid_values=invalid_values, mark_invalid=mark_invalid, invalid_color=invalid_color,
               markers=markers, marker_radius=marker_radius, marker_text_off=marker_text_off, marker_cmap=marker_cmap,
               ignore_marker_scores=ignore_marker_scores, min_marker_score=min_marker_score,
               max_marker_score=max_marker_score,
               image_range_text_off=image_range_text_off, marker_range_text_off=marker_range_text_off,
               text_off=text_off,
...
def log_image_list_NCHW(writer, desc, tensor_list, step, labels=None, idx=None, full_batch=False, every_nth=None,
                        channels='RGB', text=None, gray=False,
                        clipping=False, upper_clipping_thresh=None, lower_clipping_thresh=None,
                        mark_clipping=False, clipping_color=None,
                        invalid_values=None, mark_invalid=False, invalid_color=None,
    ...
    for tensor_idx, tensor in enumerate(tensor_list):
        ...
        log_image_NCHW(writer=writer, desc=desc, tensor=tensor, step=step, idx=label, full_batch=full_batch,
                       channels=channels, text=text, gray=gray,
                       clipping=clipping, upper_clipping_thresh=upper_clipping_thresh,
                       lower_clipping_thresh=lower_clipping_thresh,
                       mark_clipping=mark_clipping, clipping_color=clipping_color,
                       invalid_values=invalid_values, mark_invalid=mark_invalid, invalid_color=invalid_color,
...
class Logger:
    ...
    def log_in_data(self, sample, step):
        ...
        log_image_NCHW(self.writer, "{}/{}/00_images/ref".format(self.overview_base, base_name), images[0], step,
                       full_batch=full_batch)
        log_image_list_NCHW(self.writer, base_name + "/00_images", images, step, full_batch=full_batch)

        if gt_flow is not None:
            log_image_NCHW(self.writer, base_name + "/01_gt_flow", gt_flow, step, full_batch=full_batch,
                           channels='FLOW')
            log_image_NCHW(self.writer, "{}/{}/01_gt_flow".format(self.overview_base, base_name), gt_flow, step,
                           full_batch=full_batch, channels='FLOW')

        if gt_disp is not None:
            log_image_N1HW(self.writer, base_name + "/02_gt_disp", gt_disp, step, full_batch=full_batch)
            log_image_N1HW(self.writer, "{}/{}/02_gt_disp".format(self.overview_base, base_name), gt_disp, step,
                           full_batch=full_batch)
    ...
    def log_pred(self, model_output, step):
        ...
        if pred_flow is not None:
            log_image_NCHW(self.writer, base_name + "/01a_pred_flow", pred_flow, step,
                           full_batch=full_batch, channels='FLOW')
            log_image_NCHW(self.writer, "{}/{}/01_pred_flow".format(self.overview_base, base_name),
                           pred_flow, step, full_batch=full_batch, channels='FLOW')

        if pred_flows_all is not None:
            for level, pred_flow_ in enumerate(pred_flows_all):
                log_image_NCHW(self.writer, base_name + "/01b_pred_flows_all/level_%d" % level, pred_flow_,
                               step, full_batch=full_batch, channels='FLOW')

        if pred_disp is not None:
            log_image_N1HW(self.writer, base_name + "/02a_pred_disp", pred_disp, step,
                           full_batch=full_batch)
            log_image_N1HW(self.writer, "{}/{}/02_pred_disp".format(self.overview_base, base_name),
                           pred_disp, step, full_batch=full_batch)

        if pred_disps_all is not None:
            for level, pred_disp_ in enumerate(pred_disps_all):
                log_image_N1HW(self.writer, base_name + "/02b_pred_disps_all/level_%d" % level, pred_disp_,
    ...
    def log_loss(self, loss, sub_losses, pointwise_losses, step, scalars_only):
        ...
        for sub_name, sub_val in sub_losses.items():
            log_scalar(self.writer, "{}/{}".format(base_name, sub_name), sub_val, step)

        if not scalars_only:
            for pointwise_name, pointwise_loss in pointwise_losses.items():
                log_image_N1or3HW(self.writer, "{}/{}".format(base_name, pointwise_name), pointwise_loss,
                                  step, full_batch=full_batch)
    ...
    def log_metrics(self, metrics, qualitatives, step):
        ...
        for idx, (qual_name, qual) in enumerate(qualitatives.items()):
            log_image_N1or3HW(self.writer, "{}/{}_{}".format(base_name, idx, qual_name), qual, step, full_batch=full_batch)
