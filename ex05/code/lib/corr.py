...
class Corr:
    ...
    def __call__(self, feat_left, feat_right):

        if not self.cuda_corr:
            offsets = self.sampling_points.offsets.to(feat_left.device)
            corr, _ = self.corr_block(feat_ref=feat_left, feat_src=feat_right, offsets=offsets)
        else:
            corr = self.corr_block(feat_left, feat_right)
